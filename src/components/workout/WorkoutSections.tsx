...
export function PreWeekAssessment({
  data,
  assessments,
}: {
  data?: any;
  ...
  const preWeekAssessment =
    data ||
    assessments?.find(
      (a) => a.type === "baseline" && a.id === "pre_week_assessment"
    );
  ...
  return (
    <>
      ...
      <p>
        <em>{preWeekAssessment.description}</em>
      </p>
      ...
      {preWeekAssessment.attribution && (
        <div className="admonition admonition-info alert alert--info">
          ...
          <div className="admonition-content">
            <p>
              <strong>Source:</strong> {preWeekAssessment.attribution.source}
            </p>
            <p>
              <strong>Elite Use:</strong>{" "}
              {preWeekAssessment.attribution.elite_use}
            </p>
            <p>
              <strong>Methodology:</strong>{" "}
              {preWeekAssessment.attribution.methodology}
            </p>
          </div>
        </div>
      )}
      ...
      <p>
        <strong>Assessment Components ({preWeekAssessment.duration}):</strong>
      </p>
        ...
        <div key={component.id || idx} style={{ marginBottom: "2rem" }}>
          ...
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td>
                  {component.measurement?.repetitions ||
                    component.measurement?.duration}
                </td>
              </tr>
              <tr>
                <td>
              ...
              {component.standards && (
                <>
                  <tr>
                    <td>
                      <strong>Elite Benchmark</strong>
                    </td>
                    <td>{component.standards.elite_benchmark}</td>
                  </tr>
                  <tr>
                    <td>
                      <strong>Good Standard</strong>
                    </td>
          ...
          {component.context && (
            <Details summary="🎯 Assessment Context">
              <p>
                <strong>Description:</strong> {component.context.description}
              </p>
              {component.context.research && (
                <p>
                  <strong>Research:</strong> {component.context.research}
                </p>
              )}
              <p>
                <strong>Application:</strong> {component.context.application}
              </p>
            </Details>
          )}
        </div>
      ...
      {preWeekAssessment.movement_screen?.map(
        (assessment: any, idx: number) => (
          <li key={idx}>
            <strong>{assessment.name}</strong>: {assessment.reps}
            <ul>
              <li>
                <em>Elite Standard</em>: {assessment.elite_standard}
              </li>
              <li>
                <em>Professional Notes</em>: {assessment.professional_notes}
              </li>
              {assessment.research && (
                <li>
                  <em>Research Integration</em>: {assessment.research}
                </li>
              )}
              {assessment.imbalance_detection && (
                <li>
                  <em>Imbalance Detection</em>: {assessment.imbalance_detection}
                </li>
              )}
              {assessment.tennis_application && (
                <li>
                  <em>Tennis Application</em>: {assessment.tennis_application}
                </li>
              )}
              {assessment.elite_benchmark && (
                <li>
                  <em>Elite Benchmark</em>: {assessment.elite_benchmark}
                </li>
              )}
            </ul>
          </li>
    ...
    <>
      ...
      <ul style={{ listStyle: "none" }}>
        <li>
          ...
          {data.yoga_breathing?.yoga_flow?.duration}
        </li>
        <li>
          - [ ] <strong>Method:</strong>{" "}
          {data.yoga_breathing?.yoga_flow?.method}
        </li>
        <li>
          - [ ] <strong>Focus:</strong> {data.yoga_breathing?.yoga_flow?.focus}
        </li>
      </ul>

      <Details summary="💡 Professional Coaching Notes">
        <ul>
          {data.yoga_breathing?.yoga_flow?.professional_notes?.map(
      ...
      <ul style={{ listStyle: "none" }}>
        <li>
          - [ ] <strong>Duration:</strong>{" "}
          {data.yoga_breathing?.breathing_protocol?.duration}
        </li>
        <li>
          - [ ] <strong>Pattern:</strong>{" "}
          {data.yoga_breathing?.breathing_protocol?.pattern}
        </li>
        <li>
          - [ ] <strong>Technique:</strong>{" "}
          {data.yoga_breathing?.breathing_protocol?.technique}
        </li>
      </ul>

      <Details summary="🎯 Parasympathetic Activation Goals">
        <ul>
          {data.yoga_breathing?.breathing_protocol?.parasympathetic_goals?.map(
      ...
      <ul style={{ listStyle: "none" }}>
        <li>
          - [ ] <strong>Duration:</strong> {data.mindful_preparation?.duration}
        </li>
        <li>
          - [ ] <strong>Focus:</strong> {data.mindful_preparation?.focus}
        </li>
        <li>
          - [ ] <strong>Goal:</strong> {data.mindful_preparation?.goal}
        </li>
      </ul>
    </>
  );
...
export function TennisTrainingTable({ data }: { data: any }) {
  ...
  return (
    <>
      ...
      <p>
        <em>
          {data.category} | Duration: {data.total_duration} minutes | Intensity:{" "}
          {data.intensity_avg}
        </em>
      </p>

      <h3>
        🏃 Court Movement Preparation ({data.alcaraz_integration?.warm_up?.time}
        )
      </h3>
      <p>
        <strong>Method:</strong> {data.alcaraz_integration?.warm_up?.method}
      </p>
      <p>
        <strong>Standard:</strong> {data.alcaraz_integration?.warm_up?.standard}
      </p>

      {data.alcaraz_integration?.warm_up?.exercises?.map(
        (exercise: any, idx: number) => (
          <div key={idx} style={{ marginBottom: "1.0rem" }}>
            <h4>📝 {exercise.name}</h4>
            <ul style={{ listStyle: "none" }}>
              <li>
            ...
            <Details summary="💡 Detailed Instructions">
              <ul>
                ...
                {exercise.detailed_instructions?.map(
      ...
      <h3>
        🎯 Rally Development (
        {data.alcaraz_integration?.rally_development?.time})
      </h3>
      <p>
        <strong>Method:</strong>{" "}
        {data.alcaraz_integration?.rally_development?.method}
      </p>
      <p>
        <strong>Standard:</strong>{" "}
        {data.alcaraz_integration?.rally_development?.standard}
      </p>

      {data.alcaraz_integration?.rally_development?.drills?.map(
        (drill: any, idx: number) => (
          <div key={idx} style={{ marginBottom: "1.0rem" }}>
            <h4>🏆 {drill.name}</h4>
            <ul style={{ listStyle: "none" }}>
              <li>
                - [ ] <strong>Type:</strong> {drill.type}
              </li>
              <li>
                - [ ] <strong>Balls:</strong> {drill.balls}
              </li>
              <li>
            ...
            <Details summary="🥎 Forehand Protocol">
              <ul>
                {drill.forehand_instructions?.map(
            ...
            <Details summary="🥎 Backhand Protocol">
              <ul>
                {drill.backhand_instructions?.map(
                  (instruction: string, i: number) => (
                    <li key={i}>{instruction}</li>
                  )
                )}
      ...
      <h3>
        ...
        {data.alcaraz_integration?.directional_precision?.time})
      </h3>
      <p>
        <strong>Method:</strong>{" "}
        {data.alcaraz_integration?.directional_precision?.method}
      </p>
      <p>
        <strong>Standard:</strong>{" "}
        {data.alcaraz_integration?.directional_precision?.standard}
      </p>

      {data.alcaraz_integration?.directional_precision?.drills?.map(
        (drill: any, idx: number) => (
          <div key={idx} style={{ marginBottom: "1.0rem" }}>
            <h4>🎯 {drill.name}</h4>
            <ul style={{ listStyle: "none" }}>
            ...
            <Details summary="📋 Setup & Execution">
              <ul>
                {/* Handle detailed_instructions as either array of strings or array of objects */}
                {drill.detailed_instructions?.map(
                  (instruction: any, i: number) => (
                    <li key={i}>
                      {typeof instruction === "string" ? (
                        instruction
      ...
      <h3>
        ⚡ Power-Accuracy Integration (
        {data.alcaraz_integration?.power_accuracy_integration?.time})
      </h3>
      <p>
        <strong>Method:</strong>{" "}
        {data.alcaraz_integration?.power_accuracy_integration?.method}
      </p>
      <p>
        <strong>Power Range:</strong>{" "}
        {data.alcaraz_integration?.power_accuracy_integration?.power_range}
      </p>
      <p>
        <strong>Standard:</strong>{" "}
        {data.alcaraz_integration?.power_accuracy_integration?.standard}
      </p>
      ...
      <div className="admonition admonition-tip alert alert--success">
        ...
        <div className="admonition-content">
          <ul>
            {Object.entries(data.elite_success_metrics || {}).map(
...
export function StrengthTrainingSection({ data }: { data: any }) {
  ...
  return (
    <>
      <h2>
        💪 Elite Strength Foundation Block (08:40-10:00) - Murray Precision
        Integration {`{#strength}`}
      </h2>

      <p>
        <em>"{data.phase_description}"</em>
      </p>
      ...
      {data.main_exercises?.map((exercise: any, idx: number) => (
        <div key={exercise.exercise_id}>
          <h3>
            🏋️ Exercise {idx + 1}: {exercise.name}
          </h3>

          <p>
            <strong>Target:</strong> {exercise.sets} sets × {exercise.reps} reps
            @ {exercise.load} | <strong>Rest:</strong>{" "}
            {Math.floor(exercise.rest_seconds / 60)} minutes between sets
          </p>
          ...
          <ul style={{ listStyle: "none" }}>
            {exercise.set_tracking?.map((set: any, setIdx: number) => (
              <li key={setIdx}>
                - [ ] <strong>Set {set.set}:</strong> _____ lbs ×{" "}
          ...
          <Details summary="🎯 Elite Protocol Instructions">
            ...
            <ul>
              ...
              {(() => {
                ...
                if (
                  instructions.technique_points &&
                  Array.isArray(instructions.technique_points)
                ) {
                  return (
                    <>
                      {instructions.title && (
                      ...
                      {instructions.technique_points.map(
                        (instruction: any, i: number) => (
                          <li key={i}>
                            {instruction.phase ? (
                              <>
            ...
            <p>
              <strong>Professional Cues:</strong>
            </p>
            <ul>
          ...
          <Details summary="🔬 Research Integration">
            <ul>
              {exercise.research_integration?.map(
    ...
    <>
      ...
      {data.comparison_table && (
        <table>
          <thead>
            <tr>
              {data.comparison_table.headers.map(
      ...
      {data.upper_body_mobility && (
        <>
          ...
          <table>
            ...
            <tbody>
              {data.upper_body_mobility.stretches.map(
                (stretch: any, idx: number) => (
                  <tr key={idx}>
                    <td>
      ...
      {data.breathing_protocols && (
        <>
          ...
          <table>
            ...
            <tbody>
              {data.breathing_protocols.protocols.map(
      ...
      {data.morning_recovery && (
        <>
          ...
          <table>
            ...
            <tbody>
              {data.morning_recovery.activities.map(
      ...
      {data.physical_performance && (
        <>
          ...
          <table>
            ...
            <tbody>
              {data.physical_performance.metrics?.map(
      ...
      {data.movement_quality && (
        <>
          ...
          <table>
            ...
            <tbody>
              {data.movement_quality.components?.map(
      ...
      {data.tennis_performance && (
        <>
          ...
          <table>
            ...
            <tbody>
              {data.tennis_performance.skills?.map(
