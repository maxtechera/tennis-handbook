---
sidebar_position: 2
---

import WorkoutNav from '@site/src/components/WorkoutNav';

<WorkoutNav weekNumber={6} />

# Monday – Advanced Lower Body Power & Explosive Tennis

## Monday Schedule (Advanced PAP Training)

| Time | Exercise/Drill | Sets×Reps | Instructions | RPE/Velocity |
|------|---------------|-----------|--------------|-------------|
| 07:00-07:15 | Comprehensive Readiness Check | - | HRV + CNS tap test + subjective scores | - |
| 07:15–08:15 | Tennis Power Development | - | Explosive baseline patterns, max intensity rallies | 8-9/10 |
| 08:35–09:50 | Advanced PAP Complex Training | | | |
| | Back Squat (Strength) | 4×3 @ 87-92% | 3-4 min rest between sets | RPE 9/10 |
| | → Box Jumps (Power) | 4×4 | 30s after squat, max height | Explosive |
| | Bulgarian Split Squat (Heavy) | 3×5 per leg | 85% effort, explosive concentric | RPE 8/10 |

## Quick Exercise Instructions (Advanced PAP)

**Back Squat PAP Complex**: Heavy squat (87-92%) followed immediately by explosive box jumps

**Box Jumps**: Maximum height focus, soft landing, step down between reps

**Bulgarian Split Squat**: Week 6 heavy loading, explosive concentric emphasis

**Tennis Power Development**: Maximum intensity rallies, explosive movement patterns

**PAP Timing**: 30 seconds rest between strength and power exercises for optimal potentiation
| | → Lateral Bounds | 3×4 per side | 20s after split squats | Max distance |
| | Romanian Deadlift (Speed) | 4×4 @ 70% | Bar velocity ≥1.0 m/s | VBT protocol |
| | → Depth Jumps | 4×3 | 15" box, minimal ground contact | Reactive |

## Key Focus Points

- **Peak Strength Development**: 90-95% 1RM loads with pristine technique
- **PAP Protocols**: Post-activation potentiation complex training
- **Velocity-Based Training**: Bar velocity ≥0.8 m/s on speed sets
- **Competition Tennis**: 85-90% intensity with competition patterns
- **Auto-Regulation**: Green/Yellow/Red zone guidelines based on HRV

## Notes

Week 6 represents peak strength and power development using advanced training methods.

---

**Use the navigation above to move between days and weeks.**
